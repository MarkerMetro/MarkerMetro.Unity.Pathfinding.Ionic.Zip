...
// ReSharper restore UnusedMember.Local
        ...
        public ZipEntry(ZipEntry entry)
        {
            ...
            _zipFileIndex = entry._zipFileIndex;
        ...
        {
            get { return (_flags & (int) GeneralBitFlags.UnicodeText) != 0; }
            set
            {
                if (value)
                {
                    _flags |= (int) GeneralBitFlags.UnicodeText;
                }
                else
                {
                    _flags &= ~(int) GeneralBitFlags.UnicodeText;
        ...
        public int ExternalFileAttributes
        {
            get
            {
                if ((_known & Known.ExternalAttributes) == 0)
                {
                    return -1;
                }
            ...
            set
            {
                ...
                _known |= Known.ExternalAttributes;
            }
        }
        ...
        {
            get { return (_versionMadeBy & 0xff); }
        }
...
// ReSharper restore InconsistentNaming
        ...
        {
            get { return (_versionMadeBy >> 8) & 0xff; }

            set
            {
                _versionMadeBy &= 0xff;
                _versionMadeBy |= (ushort) ((value & 0xff) << 8);
            }
        }
        ...
        public int Version
        {
            get
            {
                ...
                else if (CompressionMethod.Deflated == _method)
                {
                    result = 20;
                }
                else if (IsDirectory)
        ...
        public bool LocalHeaderRequiresZip64
        {
            get
            {
                bool result = _forceZip64;

                if (!result)
                {
                    ulong trueCompressedSize = _compressedSize;
                    ...
                    result =
                        ((_size >= uint.MaxValue) || (trueCompressedSize >= uint.MaxValue)) &&
                        ((_versionToExtract == 0) || (_versionToExtract >= ZipConstants.VersionZip64));
                }
        ...
        {
            get { return LocalHeaderRequiresZip64 || (_offset >= uint.MaxValue); }
        }
        ...
        public DateTime DateTime
        {
            get
            {
                uint sec = Math.Min(59, 2*(_dosTime & 0x1f));
                uint min = Math.Min(59, (_dosTime >> 5) & 0x3f);
                uint hrs = Math.Min(23, (_dosTime >> 11) & 0x1f);
                uint mon = Math.Max(1, Math.Min(12, ((_dosTime >> 21) & 0xf)));
                uint year = ((_dosTime >> 25) & 0x7f) + 1980;
                int day = Math.Max(1,
                                   Math.Min(DateTime.DaysInMonth((int) year, (int) mon), (int) ((_dosTime >> 16) & 0x1f)));
        ...
        public long Size
        {
            get { return (_known & Known.Size) != 0 ? (long) _size : -1L; }
            set
            {
                _size = (ulong) value;
                _known |= Known.Size;
        ...
        {
            get { return (_known & Known.CompressedSize) != 0 ? (long) _compressedSize : -1L; }
            set
            {
                _compressedSize = (ulong) value;
                _known |= Known.CompressedSize;
            }
        }
        ...
        public long Crc
        {
            get { return (_known & Known.Crc) != 0 ? _crc & 0xffffffffL : -1L; }
            set
            {
                if ((_crc & 0xffffffff00000000L) != 0)
                {
        ...
        {
            get { return (AESKeySize > 0) ? CompressionMethod.WinZipAES : _method; }
        }
...
// ReSharper restore InconsistentNaming
        ...
        {
            var extraData = new ZipExtraData(_extra);
            ...
            if (extraData.Find(0x0001))
            {
                ...
                if (localHeader || (_size == uint.MaxValue))
                {
                    _size = (ulong) extraData.ReadLong();
                }

                if (localHeader || (_compressedSize == uint.MaxValue))
                {
                    _compressedSize = (ulong) extraData.ReadLong();
                }

                if (!localHeader && (_offset == uint.MaxValue))
                {
                    _offset = extraData.ReadLong();
                }
            ...
            {
                if (
                    ((_versionToExtract & 0xff) >= ZipConstants.VersionZip64) &&
                    ((_size == uint.MaxValue) || (_compressedSize == uint.MaxValue))
                    )
                {
            ...
            else if (extraData.Find(0x5455))
            {
                ...
                int flags2 = extraData.ReadByte();

                // Can include other times but these are ignored.  Length of data should
                // actually be 1 + 4 * no of bits in flags.
                if (((flags2 & 1) != 0) && (length >= 5))
                {
        ...
        public bool IsDirectory
        {
            get
            {
                ...
                bool result =
                    ((nameLength > 0) &&
                     ((_name[nameLength - 1] == '/') || (_name[nameLength - 1] == '\\'))) ||
                    HasDosAttributes(16)
                    ;
                return result;
            }
        ...
        public object Clone()
        {
            ...
            if (_extra != null)
            {
                result._extra = new byte[_extra.Length];
                Array.Copy(_extra, 0, result._extra, 0, _extra.Length);
            }
        ...
        private int _externalFileAttributes = -1; // contains external attributes (O/S dependant)

        private ushort _versionMadeBy; // Contains host system and version information
        // only relevant for central header entries

        private readonly string _name;
        private ulong _size;
        private ulong _compressedSize;
        private readonly ushort _versionToExtract; // Version required to extract (library handles <= 2.0)
        private uint _crc;
        ...
        private int _flags; // general purpose bit flags

        private long _zipFileIndex = -1; // used by ZipFile
        private long _offset; // used by ZipFile and ZipOutputStream
